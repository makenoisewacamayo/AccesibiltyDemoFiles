    ...
    //var nm_empty_zones=[0,0,0,0];
    var nm_empty_zones=[0,0,0,0,0];
	try{
		if (OA_output["POR_300x560_A"].indexOf('bannerid=0') > 0){
			nm_empty_zones[0]=1;
		}
	}catch(error){}

	try{
		if (OA_output["POR_300x560_B"].indexOf('bannerid=0') > 0){
			nm_empty_zones[1]=1;
		}
	}catch(error){}

	try{
		if (OA_output["POR_300x560_C"].indexOf('bannerid=0') > 0){
			nm_empty_zones[2]=1;
		}
	}catch(error){}

	try{
		if (OA_output["POR_300x560_D"].indexOf('bannerid=0') > 0){
			nm_empty_zones[3]=1;
		}
	}catch(error){}
	
	try{
		if (OA_output["POR_300x560_E"].indexOf('bannerid=0') > 0){
			nm_empty_zones[4]=1;
		}
	}catch(error){}
    ...
    {    
        ...
        $('#nm_zone_inner_' + nm_active_pos).animate({opacity: '1'},500);
   ...
   function nm_count()
   {
        ...
        if (nm_countTime == 0.5)
        {   
            $('#nm_zone_inner_' + nm_active_pos).animate({opacity: '0.2'},500);
        }
        if (nm_countTime == 0)
        {   
    ...
    function nm_activate_ads_pos(nm_pos_to_active, nm_num_ads){
        ...
        for (i = 1; i <= nm_num_ads; i++) { 
            if(i!=nm_pos_to_active){
                ...
                $('#nm_zone_inner_' + nm_pos_to_active).animate({opacity: '1'},500);
	...
	$(document).ready(nm_startDelay(nm_delay));